...
def srno(request):
    ...
    if request.method == 'GET':
        ...
        if export_type == 'excel':
            ...
            # Convert the HTML table directly to Excel
            html_table = '<html><body>' + df.to_html(index=True, escape=False, classes='table table-striped') + '</body></html>'
            df_html = pd.read_html(html_table)[0]
            
            df_html.to_excel(writer, sheet_name='Report', index=False)
            writer.save()
