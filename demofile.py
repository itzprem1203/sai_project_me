...
class mainGUI:
    def __init__(self):
        self.window = tk.Tk()
        self.window.title("GUI UART Tx/Rx Demo")
        ...
        entryBaudrate.grid(row=1, column=4, padx=5, pady=3)
        ...
        self.data_buffer = deque(maxlen=11)  # buffer to keep last 11 pieces of data
        self.window.mainloop()
    ...
    def readUART(self):
        while True:
            if self.uartState:
                try:
                    data = self.ser.read(self.ser.in_waiting or 1).decode('ascii')
                    if data:
                        self.data_buffer.extend(data.splitlines())
                        # Ensure the buffer has only the last 11 pieces of data
                        while len(self.data_buffer) > 11:
                            self.data_buffer.popleft()
                        # Update the OutputText widget
                        self.OutputText.delete(1.0, tk.END)  # Clear previous data
                        self.OutputText.insert(tk.END, " | ".join(self.data_buffer))  # Insert new data as a single line
                        print(data, end='')
                except Exception as e:
                    informStr = f"Something wrong in receiving: {e}"
                    InformWindow(informStr)
